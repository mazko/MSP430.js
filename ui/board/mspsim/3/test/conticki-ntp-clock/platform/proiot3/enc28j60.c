...
static char bsp_spi_send_byte(const char data) {
...
static void bsp_spi_select() {
  P5OUT &= ~1;
}

static void bsp_spi_release() {
  while(!(U1TCTL&TXEPT));
  P5OUT |= 1;
}

#define enc28j60_select()  bsp_spi_select()
#define enc28j60_release() bsp_spi_release()

#define enc28j60_rx()     bsp_spi_send_byte(0xff)
#define enc28j60_tx(data) bsp_spi_send_byte(data)
...
void enc28j60_init(const uint8_t macadr[6])
{
  // Reset ENC28J60
  enc28j60_soft_reset();
...
tapdev_send(const uip_lladdr_t *lladdr)
{
  ...
  enc28j60_send_packet(uip_buf, uip_len);
  return 0;
}
#else
#define tapdev_send() enc28j60_send_packet(uip_buf, uip_len)
#endif
#define tapdev_poll() enc28j60_recv_packet(uip_buf, sizeof uip_buf)
/*---------------------------------------------------------------------------*/
#if !NETSTACK_CONF_WITH_IPV6
uint8_t
tapdev_output(void)
